...
class MoRRouter(nn.Module):
    ...
    def init(self, input_dim: int, hidden_dim: int = 256, max_recursions: int = 3, decision_threshold: float = 0.5, use_attention: bool = False):
        super().__init__()
    ...
    def buildattention_gate(self) -> nn.Module:
        ...
        return nn.MultiheadAttention(embed_dim=self.input_dim, num_heads=8, dropout=0.1, batch_first=True)
    ...
    def get_routing_signal(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        recursion_emb = self.recursion_embeddings(torch.tensor(self.recursion_count, device=hidden_states.device))
        routing_signal = routing_signal + recursion_emb
        return routing_signal

    def forward(self, hidden_states: torch.Tensor, layer_fn: Callable, next_layer_fn: Optional[Callable] = None, training: bool = True) -> torch.Tensor:
        ...
        for batch_idx in range(hidden_states.size(0)):
            ...
            if decision_probs[0] > 0.5 and self.recursion_count < self.max_recursions:
                ...
                output[batch_idx] = self.forward(hidden_states[batch_idx:batch_idx+1], layer_fn, next_layer_fn, training)[0]
                self.recursion_count -= 1  

            elif decision_probs[1] > 0.5 and next_layer_fn is not None:
                # Forward: Apply next layer
                output[batch_idx] = next_layer_fn(hidden_states[batch_idx:batch_idx+1])[0]
                
            else:
                # Skip or default: Apply current layer once
                output[batch_idx] = layer_fn(hidden_states[batch_idx:batch_idx+1])[0]
        return output
    ...
    def get_routing_stats(self) -> Dict[str, float]:
        ...
        if self.total_decisions > 0:
            recursion_rate = (self.recurse_decisions.float() / self.total_decisions.float()).item()
        else:
            recursion_rate = 0.0
        return {'total_decisions': self.total_decisions.item(), recurse_decisions': self.recurse_decisions.item(), 'recursion_rate': recursion_rate
        }
